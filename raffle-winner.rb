...
get '/' do
  ...
  response = RestClient.get 'http://lsrc.iriscouch.com/raffle2011/_design/random/_view/all'
  count = JSON.parse(response.to_str)["total_rows"]

  # find a random document
  r = Random.new
  i = count > 1 ? r.rand(1...count) : count - 1
  response = RestClient.get "http://lsrc.iriscouch.com/raffle2011/_design/random/_view/all?reduce=false&skip=#{i}&limit=1"
  ...
  response = RestClient.put 'http://lsrc.iriscouch.com/raffle2011/' + phone, "{\"_rev\": \"#{rev}\", \"name\": \"#{@name}\", \"Won\": true}", {:content_type => :json}
  haml :index
end
